...
If routing to a port on an external service, the port must be exposed on that service (either as the default port or through a path routing rule).
  ...
  - **appName** \- (optional) the name of the application to route to. Defaults to the name of the web service's application.
  - **serviceName** \- (optional) the name of the service to route to. Defaults to the name of the web service. MUST be specified if appName is set.
