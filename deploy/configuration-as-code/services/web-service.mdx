...
- [pathRoutingConfig](#pathroutingconfig) \- optional configuration options for path-based routing.
  - **rewriteMode** \- mode for rewriting URL paths. Options are `rewrite-all` (default), `rewrite-prefix`, and `rewrite-off`.
    - `rewrite-all` \- rewrite the entire path to the root path.
    - `rewrite-prefix` \- rewrite the path for the URL path prefix only.
    - `rewrite-off` \- disable path rewriting.
