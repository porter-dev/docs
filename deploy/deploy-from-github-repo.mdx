...
**Fixed instances** runs a constant number of replicas. Set this to 3 or more for high availability. If one instance fails, others continue serving traffic while a replacement starts.

**Resource-based autoscaling** automatically adjusts the number of instances based on CPU and memory utilization. You configure a minimum and maximum instance count, along with target utilization percentages. The service will scale up when either CPU or memory utilization exceeds the configured threshold.
