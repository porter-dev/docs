...
- **Waiting**: "Waiting for partner apps: app-name-1, app-name-2"
- **Failed**: "Failed to deploy because paired app backend didn't deploy the required version"
- **Success**: "Deployment completed successfully for all apps in the group"

The dashboard provides visibility into exactly why a deployment might be pending or failing.

![Blue-Green Status in Activity Feed](/images/blue-green/bg-activity-feed-status.png)
...
1. **Same Image Tag Requirement**: All applications in a Blue-Green group must be deployed with the same image tag. This is typically the commit SHA for applications deployed from GitHub.

2. **Deployment Time Window**: All applications in a group must complete their deployment within a 20 min time window. If one application fails to deploy or takes too long, the entire group deployment is considered failed and reverted.

3. **Configuration Consistency**: All applications in the Blue-Green group should have the `blueGreen` strategy and the same group name configured.

## Example Use Case: API and Web Application

Consider a scenario with a web application and its backend API that must stay compatible:
