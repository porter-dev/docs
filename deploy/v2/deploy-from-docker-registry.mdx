...
This guide covers deploying pre-built container images. If you want Porter to build your application from source code, see [Deploy from a GitHub repository](/deploy/v2/deploy-from-github-repo).
...
![Service type selection dropdown](/images/deploy/v2/docker-service-type-dropdown.webp)
...
![CPU and RAM sliders](/images/deploy/v2/docker-cpu-ram-sliders.webp)

#### Scaling

Choose between fixed instances (always run N copies), resource-based autoscaling (scale based on CPU or memory utilization), or [custom autoscaling](/configure/autoscaling) for metric-driven scaling.

![Autoscaling configuration](/images/deploy/v2/autoscaling-config.webp)
...
Add your domain in the Networking section, then configure DNS to point to your cluster's load balancer (displayed in Porter with a copy button). For GKE and AKS clusters, create an A record pointing to the static IP. For EKS clusters, create a CNAME record pointing to the load balancer hostname. SSL certificates are provisioned automatically.

![Custom domain configuration with DNS instructions](/images/deploy/v2/docker-custom-domain-dns.webp)
...
![Environment variables accordion](/images/deploy/v2/env-vars-accordion.webp)
...
![Health check configuration](/images/deploy/v2/docker-health-check-config.webp)
...
![Worker service configuration](/images/deploy/v2/docker-worker-config.webp)
...
Configure a job service when your container should run on a schedule. Specify a cron expression for the schedule (all times are in UTC). Porter shows a human-readable description as you type.

![Job with cron schedule](/images/deploy/v2/docker-job-cron-config.webp)
