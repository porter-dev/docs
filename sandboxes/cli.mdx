...
| `--env-group` | Name of an [environment group](/applications/configure/environment-groups) on the cluster whose variables are injected into the sandbox. Repeatable |
...
`--env-group` attaches an [environment group](/applications/configure/environment-groups) on the cluster to the sandbox. Repeat the flag to attach several groups. Values are snapshotted at create time and merged with any explicit `--env` values: a later group wins on key conflicts between groups, and an explicit `--env` value wins over any group value. Create fails with a `400` if a named group does not exist or has not been populated yet.
