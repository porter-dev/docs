...
| `volume.writeFile(path, content)` | Writes bytes to a file, creating parent directories and replacing any existing file. Atomic. |
| `volume.writeText(path, content)` | Writes a string to a file. See `writeFile`. |
