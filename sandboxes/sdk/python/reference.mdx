...
| `volume.write_file(path, content)` | Writes bytes to a file, creating parent directories and replacing any existing file. Atomic. |
| `volume.write_text(path, content, encoding="utf-8")` | Writes a string to a file. See `write_file`. |
| `volume.move_file(from_path, to_path)` | Moves or renames a file or directory; `to_path` is the full new path. Fails if the destination is occupied. |
