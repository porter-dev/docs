...
CIDR entries are honored as written, including private address space, so an allowlist can deliberately grant a sandbox access to internal IPs that the default isolation blocks. In-cluster Kubernetes Services have a wrinkle: connections to a ClusterIP are rewritten to a backing pod IP before the allowlist is evaluated, so a service hostname or ClusterIP entry won't match. To reach an in-cluster service, list the IPs of its backing pods, and keep the entry current as those pods change.
