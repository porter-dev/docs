---
title: "Custom Metrics and Autoscaling"
description: "Learn how to configure custom metrics collection and autoscaling in Porter"
---
...
<Callout type="info">
  Metrics scraping is only available for web services.
</Callout>
...
<Callout>
  Our telemetry collector will automatically send requests to the specified port and path to collect metrics from your service.
</Callout>
...
<Callout type="warning">
  If your query returns multiple values or time series, use aggregation operators like `avg()`, `sum()`, or `max()` to reduce it to a single value.
</Callout>
...
<Tabs items={['App 1: Analytics Ingestion API', 'App 2: Event Processing Service']}>
  <Tab>
