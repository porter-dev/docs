...
Therefore, you simply have to create an `A` record in your DNS provider that points to that static IP address.
...
Instead, you need to create a `CNAME` record that points at the Elastic IP address. For subdomains (e.g. domains that follow the format of `*.porter.run`), this is straightforward. For apex domains (e.g. `porter.run`), however,
