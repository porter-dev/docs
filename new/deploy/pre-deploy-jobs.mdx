...
(See our other doc for information on how to start writing a `porter.yaml`)

If you have a `porter.yaml` file configured for your application, adding a pre-deploy job is as easy as adding a `predeploy` section to your `porter.yaml` file:

```yaml
# porter.yaml
services: ...
predeploy:
  run: bash ./migrate-db.sh
```
