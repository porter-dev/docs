...
Porter supports autoscaling based on custom metrics (e.g. queue latency, queue length, etc.). Your cluster comes with [Prometheus](https://prometheus.io/) installed by default, which collects various metrics available on your cluster.

For detailed instructions on setting up custom metrics and autoscaling, see our [Custom Metrics and Autoscaling](/observability/custom-metrics-and-autoscaling) guide.