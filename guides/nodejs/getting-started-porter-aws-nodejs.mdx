...
While Porter's a great way to quickly get started with pushing out your app, there are certain limitations. For one, your app runs on our infrastructure, where there are certain limits on the amount of resources you can use. Further, since it's not your own infrastructure at play, it can be troublesome from a compliance perspective. With a successful app, comes a lot of responsibility! 
...
We're going to deploy a sample Nodejs server written in [Express.js](https:/expressjs.com/) - but that doesn't mean you can't bring _any_ Javascript framework to the table. On the contrary - we're open to any and all frameworks! This app's a very basic app with two endpoints - `/` and a basic healthcheck endpoint at `/healthz`, to demonstrate how you can push out a public-facing app on Porter with all bells and whistles attached and ready to go. The idea here is to show you how a basic app can be quickly deployed on Porter, allowing you to then use the same flow for deploying your code. 
...
4. Optionally - telling your colleagues and friends about your new app and all it can do, and spreading the word!

Let's fire up this furnace.
...
Now that Porter can interact with your AWS account - the real work begins. You'll see a screen with a form - the fields are pre-filled, with details about your cluster:

![5-cluster-setup](images/guides/getting-started-porter-aws-nodejs/5-cluster-setup.png)

Ordinarily, the only two fields you would find worth tweaking are the cluster name and the region - feel free to change those. The other fields are usually changed if Porter detects a conflict between the proposed cluster's VPC and other VPCs in your account - these would be flagged during a preflight test, allowing you the option of tweaking those addresses. You can also choose a different instance type in this section for your cluster; while we tend to default to `t3.medium` instances, we have a lot more instance types. Once you're satisfied, click `Deploy`.
...
Deploying your cluster can take up to 20 minutes - enough time to brew up a pot of tea or a cup of coffee(or rocket fuel as it's known sometimes).
...
You're almost there! Hitting `Deploy` will show you the contents of a Github Action workflow that Porter would use to build and deploy your app:
...
All you need to do - merge this PR and watch your build go through!
...
Progress! Your app's now live on Porter. The Porter-generated unique URL is now visible on the dashboard, under your app's name. Let's take it for a spin:

![19-access-web-app](/images/guides/getting-started-porter-aws-nodejs/19-access-web-app.png)

Things look pretty neat here. You can also check logs and resource consumption metrics on the Porter dashboard to see how your app's faring:
...
We've seen how you can go about deploying your Nodejs app on Porter. But there's so much more to running an app which Porter helps you with - here are a few pointers to help you dive further into configuring/tuning your app(and of course, going viral!):

1. [Adding your own domain](/configure/custom-domains). Names _matter_!. 
2. [Changing instance types](/provision/changing-instance-types). Sometimes, you do need that extra boost to get past the finish line - ask Dom Toretto.
3. [Adding environment variables and groups](/configure/environment-groups). There's a reason environment variables have truly passed the test of time.
4. [Scaling your app](/configure/autoscaling). Because your app should never be slow enough to give your users time to make coffee. 
5. [Ensuring your app's never offline](/configure/health-checks). It's like seatbelts..they seem like a bit much, but _so_ underrated. 

The best part? If you ever need help with your app - you can always reach out using the chat widget on our dashboard. Nothing makes us happier than helping our users _not_ worry about their infrastructure. 