---
title: "Deploy a Next.JS App on GCP with Porter"
---
...
That's where Porter Standard comes in - it allows you to experience the same ease-of-use of the Porter experience, but in your _own_ cloud account. You get the flexibility of running your own industry-grade infrastructure, without actually worrying about how it's provisioned or managed - Porter takes care of all those details for you. In this guide, we're going to walk through using Porter to provision infrastructure in a GCP project, and then deploy a basic Nodejs app and have it up and running.

Note that to follow this guide, you'll need an account on [Porter Standard](https:/dashboard.getporter.dev) along with a GCP project. There is a base charge levied by GCP for a basic cluster provisioned by Porter - this would be around $250 per month, and is subject to change if you run more resource-intensive apps/workloads.
...
![4-gcp-cluster-creation-provisioning](images/guides/porter-gcp/4-gcp-cluster-creation.png)
