...
The default Porter-managed Kubernetes cluster on AWS using a Network Load Balancer (NLB) which forward traffic to an on-cluster Nginx Ingress Controller.
...
- `Cluster Name` can be found in the `Infrastructure` or `Cluster Settings` tab of your Porter Dashboard
...
1. In your AWS console, navigate to the region that your cluster was deployed in. You can find this from your Porter dashboard in the `Infrastructure` or `Cluster Settings` in the Configuration tab.
...
1. \_Ensure that `Enabled TLS and automatically create certificates` is UNCHECKED`
