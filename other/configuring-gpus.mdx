  ...
  <Step title="Configure the GPU node group">
    ...
    <Tabs>
      <Tab title="AWS">
        ...
        |---------------|-----|------------|----------|
        | `g4dn.xlarge` | 1x NVIDIA T4 | 16 GB | Inference, light training |
        | `g4dn.2xlarge` | 1x NVIDIA T4 | 16 GB | Inference with more CPU/RAM |
        | `g5.xlarge` | 1x NVIDIA A10G | 24 GB | Training, larger models |
        | `g5.2xlarge` | 1x NVIDIA A10G | 24 GB | Training with more CPU/RAM |
        | `p3.2xlarge` | 1x NVIDIA V100 | 16 GB | High-performance training |
      </Tab>

      <Tab title="Azure">
        | Instance Type | GPU | GPU Memory | Use Case |
        |---------------|-----|------------|----------|
        | `Standard_NC4as_T4_v3` | 1x NVIDIA T4 | 16 GB | Inference, light training |
        | `Standard_NC8as_T4_v3` | 1x NVIDIA T4 | 16 GB | Inference with more CPU/RAM |
        | `Standard_NC6s_v3` | 1x NVIDIA V100 | 16 GB | High-performance training |
        | `Standard_NC24s_v3` | 4x NVIDIA V100 | 64 GB | Multi-GPU training |
      </Tab>

      <Tab title="Google Cloud">
        | Instance Type | GPU | GPU Memory | Use Case |
        |---------------|-----|------------|----------|
        | `g2-standard-4` | 1x NVIDIA L4 | 24 GB | Inference, light training |
        | `g2-standard-8` | 1x NVIDIA L4 | 24 GB | Inference with more CPU/RAM |
        | `n1-standard-4` + T4 | 1x NVIDIA T4 | 16 GB | Cost-effective inference |
        | `a2-highgpu-1g` | 1x NVIDIA A100 | 40 GB | High-performance training |
      </Tab>
    </Tabs>

    <Warning>
      GPU instances are significantly more expensive than standard instances. Set minimum nodes to 0 if your workload can tolerate cold starts, allowing nodes to scale down when not in use.
    </Warning>
  </Step>
...
To verify your application has GPU access, you can check from within your container:
...
You should see output showing your GPU model, memory usage, and any running processes.

If using PyTorch or TensorFlow, you can verify GPU access in your application code:
...
If your GPU workload is intermittent (e.g., batch inference, scheduled training), set minimum nodes to 0. Your GPU nodes will scale down when not in use, potentially saving hundreds of dollars per month.

<Info>
  Scale-to-zero adds cold start latency (typically 2-5 minutes) when nodes need to spin up. For latency-sensitive workloads, keep at least one node running.
</Info>

### Choose the Right Instance Size

- **Inference**: Start with smaller instances like `g4dn.xlarge` (AWS) or `g2-standard-4` (GCP)
- **Training**: Use larger instances with more GPU memory like `g5.xlarge` or `a2-highgpu-1g`
- **Multi-GPU**: Only use multi-GPU instances if your framework supports distributed training

### Monitor GPU Utilization

Use Porter's monitoring to track GPU utilization. If utilization is consistently low, consider:
- Using a smaller instance type
- Batching requests for better throughput
- Reducing minimum nodes if you have idle capacity
...
<AccordionGroup>
  ...
  <Accordion title="GPU not detected in container">
    ...
    - You've assigned the service to the GPU node group
    - You've requested GPU resources in the service configuration
    - The container image has NVIDIA drivers/CUDA installed
  </Accordion>
  ...
  <Accordion title="Slow cold starts">
    ...
    - Use preemptible/spot instances for cost savings if cold starts are acceptable
