...
<Steps>
  ...
  <Step title="Configure the GPU node group">
    ...
    <Warning>
      GPU instances are significantly more expensive than standard instances. Set minimum nodes to a low amount so nodes can scale down when not in use. Note that a low minimum nodes can add cold start latency (typically 2-5 minutes) when nodes need to spin up.
    </Warning>
  </Step>
