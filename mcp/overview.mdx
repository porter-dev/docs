...
One command installs the Porter CLI, registers this server with the agent clients it finds, and installs Porter's skills:

```bash
curl -fsSL https://agents.porter.run | sh
```

Restart your client afterward. It opens the browser for sign-in the first time it calls a Porter tool.

Run from inside an agent session, the installer configures that client alone; `--clients=all` configures every client on the machine. Add `--help` for the rest, `--dry-run` to preview, and `--uninstall` to remove everything it added.

The skills it installs teach an agent a complete Porter workflow, beyond what the individual [tools](/mcp/tools) describe. Today that means `porter-deploy`, which takes a GitHub repository from nothing to a live URL and follows the generated workflow pull request through merge.
