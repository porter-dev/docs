...
```bash
az aks show -g <resource-name> -n <region> --query 'networkProfile.loadBalancerProfile.effectiveOutboundIPs[].id'
```

You need to replace `<resource-name>` with the name of the resource group your cluster is deployed into and `region` with the region your cluster is deployed.
...
```
where `<id>` is the ID you got from the previous command (preferably between quotes).
