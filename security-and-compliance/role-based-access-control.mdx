...
description: "Assign admin, developer, or viewer roles to team members with SSO integration and just-in-time user provisioning for Porter projects"
---
...
## Security tab access[](#security-tab-access "Direct link to heading")

The **Security** section in the sidebar contains three tabs: **Audit logs**, **Compliance**, and **Trust center**. Access depends on your project role:

* **Admin:** access to all three tabs. Opening **Security** lands on **Audit logs** by default.
* **Developer and Viewer:** access to **Compliance** and **Trust center** only. Opening **Security** lands on **Compliance** by default. Navigating directly to an audit log URL renders an unauthorized state, and no audit log data is fetched.

To grant a teammate access to audit logs, change their role to **Admin** from the **Settings** tab. See [Changing collaborator permissions](#changing-collaborator-permissions).
