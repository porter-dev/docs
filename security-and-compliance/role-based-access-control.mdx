...
## Just In Time Single Sign On[](#just-in-time-single-sign-on "Direct link to heading")

To change an invite or collaborator role, you must be logged in with an **Admin** role. As an admin, navigate to **Settings** then **Members** in the sidebar. Finally, click the **SSO** tab to view your current just in time single sign on integration.

To set up SSO, you will need to click the **Set Up** button. You will then be prompted to login with an account. Ensure that the account you log in is not your personal email and is connected to your organization.

Once logged in, any new users in the same organization will automatically be added to the project without an invite email. By default these users will receive **viewer** role, but you can also configure the default role. You can also click **Update Provider** to set up a different organization.
