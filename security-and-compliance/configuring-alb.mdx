---
title: "Configuring AWS ALB on Porter"
sidebarTitle: "Configuring AWS ALB"
description: "Switch from NLB to AWS Application Load Balancer on Porter for IP allowlisting, WAFv2 integration, ACM certificates, and DNS validation"
---

<Info>
  This page covers swapping the cluster's NLB for an ALB, which is kept for clusters already running on it. To set up an ALB now, add one alongside the default load balancer instead, see [Application load balancer](/cloud-accounts/advanced-cluster-settings#application-load-balancer). The ACM validation steps below apply either way.
</Info>
...
1. You can now use this as a custom domain in Porter for your applications. See [Deploying on the Custom Domain](/applications/configure/custom-domains#custom-domains) to setup your new domain to be compatible with Porter.
