...
| `loadBalancerConfig` | object | No | Deprecated. Single load balancer fronting the service |
...
<Info>
`private` is shorthand for `loadBalancers: [private_lb]`. Prefer `loadBalancers` for new services, and note that setting both on the same service is not supported.
</Info>
...
Lists the load balancers that serve the web service. Each entry is a mode, written as a bare string:
...
A service can list several load balancers at once, which is how you move a domain between them without downtime: serve both, repoint the domain's DNS record, then drop the one you're leaving. An empty list means the service is only reachable in-cluster.
...
<Warning>
  Deprecated. Use `loadBalancers`, which can front a service with several load balancers at once.
</Warning>

Configures the single load balancer that fronts the web service. Use this to explicitly route the service through the cluster's public or private load balancer.
