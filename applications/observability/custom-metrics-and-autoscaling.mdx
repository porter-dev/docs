...
For other autoscaling options, see the [Autoscaling overview](/applications/configure/autoscaling).

## Configuring Metrics Scraping

**Note:** The metrics scraping toggle is only available for web services. However, metric-based autoscaling can be configured on any service type (web or worker) as long as the metric is available in Prometheus — for example, via a Helm chart addon that exposes metrics.
