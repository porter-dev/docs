...
Porter uses [GKE Workload Identity](https://cloud.google.com/kubernetes-engine/docs/concepts/workload-identity) to bind a service's Kubernetes service account to a GCP IAM service account in your project. Your application code can then use [Application Default Credentials](https://cloud.google.com/docs/authentication/application-default-credentials) (e.g. `google.auth.default()` in Python, `google.NewClient` in Go, `GoogleCredentials.getApplicationDefault()` in Java) and GCP will issue tokens for the bound IAM service account on the pod's behalf.

### How it works

When you attach a GCP IAM service account to a service, Porter adds an IAM policy binding on the service account granting `roles/iam.workloadIdentityUser` to the Kubernetes service account Porter creates for your service. At runtime, the pod presents a projected OIDC token via the GKE metadata server and GCP exchanges it for an access token scoped to the IAM service account.
...
In the same GCP project as your GKE cluster, create an IAM service account and grant it only the IAM roles needed to access the specific resources your service will call.
...
You don't need to add the `roles/iam.workloadIdentityUser` binding yourself — Porter adds it on the next deploy.
...
On the next deploy, Porter adds the workload identity binding on the IAM service account. From your application code, use the standard Google Cloud client libraries and credentials will be resolved automatically — no service account key files, no `GOOGLE_APPLICATION_CREDENTIALS` env var.
...
- [GKE Workload Identity](https://cloud.google.com/kubernetes-engine/docs/concepts/workload-identity) — GCP documentation
