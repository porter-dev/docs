...
To deploy your application, first navigate to the **Launch** tab, where you will see three types of applications to deploy: web, worker, or job. To understand the difference between these three application types, see the description of the [types of applications](/enterprise/deploying-applications/overview#types-of-applications). After selecting your application type and selecting **Launch Template**, you should select the **Docker Registry** option:
...
<Tabs>
  <Tab title="Private">
    To deploy from a private image, make sure you have [linked a private registry](/enterprise/deploying-applications/deploying-from-docker-registry/linking-existing-registry). Then, simply select the image repo and tag you would like to use:
