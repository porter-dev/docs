...
The CLI also supports deploying directly from a Docker image which is hosted on a [connected Docker registry](/enterprise/deploying-applications/deploying-from-docker-registry/linking-existing-registry). Simply specify `--source registry` and the application image via the `--image` tag:
