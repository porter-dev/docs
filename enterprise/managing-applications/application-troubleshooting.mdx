...
You can read more about the start command for web applications in the [runtime configuration options](/enterprise/deploying-applications/runtime-configuration-options/web-applications#start-command).

One method to check which commands are set in the `$PATH` of your container is to set the start command to `sleep infinity`, and then use the [porter run command](/enterprise/cli/remote-shell-access) to get shell access. From this shell, you could for example run:
...
* `137`: indicates that the process was killed by `SIGKILL`. The most common reason for this is that your application does not handle graceful shutdown when it receives a `SIGTERM` signal. After receiving `SIGTERM`, your application should close existing connections and terminate with exit code `0`. See the [graceful shutdown doc](/enterprise/deploying-applications/zero-downtime-deployments#graceful-shutdown) for more information.
...
* You have connected an image registry to Porter. For more information, see the [getting started guide](/enterprise/getting-started/overview) or the doc on [deploying from a Docker registry](/enterprise/deploying-applications/deploying-from-docker-registry)
...
* If your requests take longer than 30 seconds to resolve, or you are running a websocket-based application, the default read and write timeouts may not be long enough. See the [networking configuration doc](/enterprise/managing-applications/network-configuration-options#setting-custom-readwrite-timeouts) for how to increase the read/write timeouts.
* If your application sends very large headers as part of the response body, consider increasing the response header size, as documented [here](/enterprise/managing-applications/network-configuration-options#header-size).
* If you are seeing dropped connections while redeploying, follow the instructions for [zero-downtime deployments](/enterprise/deploying-applications/zero-downtime-deployments).
...
This is caused by the NGINX instance rejecting requests that are too large. See the [networking configuration doc](/enterprise/managing-applications/network-configuration-options#client-max-body-size) for how to resolve these errors.
