---
title: "Quickstart"
description: "Deploy your first application on Porter from sign-up to a live public URL, covering cloud account setup, cluster provisioning, and app creation"
---
...
<Tabs>
  ...
  <Tab title="GCP">
    Porter connects to GCP using [Workload Identity Federation](https://cloud.google.com/iam/docs/workload-identity-federation). Porter's AWS identity exchanges short-lived (≤1 hour) tokens with your GCP project on demand — no service account keys to download, store, or rotate.

    <Steps>
      <Step title="Enter your GCP Project ID">
        In Porter, select GCP. Enter your [GCP project ID](https://console.cloud.google.com) (you can find this at the top of any page in the GCP Console) and click **Connect**.
      </Step>
      <Step title="Run the setup in Cloud Shell">
        ...
        The script runs Terraform inside your project and creates a service account, a Workload Identity Pool with a trust policy scoped to your tenant, and enables the five APIs Porter needs to bootstrap federation. Setup takes about 30 seconds.
      </Step>
      <Step title="Wait for verification">
        The Porter dashboard polls automatically. Once the bootstrap completes, Porter finishes enabling the remaining APIs, grants the heavier IAM roles to the service account, and marks the cloud account connected. The status banner turns green and the Continue button enables.
