...
You can read more about the start command for web applications in the [basic configuration](/configure/basic-configuration#setting-the-start-command).
...
* `137`: indicates that the process was killed by `SIGKILL`. The most common reason for this is that your application does not handle graceful shutdown when it receives a `SIGTERM` signal. After receiving `SIGTERM`, your application should close existing connections and terminate with exit code `0`. See the [graceful shutdown doc](/configure/zero-downtime-deployments#graceful-shutdown) for more information.
...
* You have connected an image registry to Porter. For more information, see the [deploy overview guide](/deploy/overview) or the doc on [deploying from a Docker registry](/deploy/v2/deploy-from-docker-registry)
...
* If your requests take longer than 30 seconds to resolve, or you are running a websocket-based application, the default read and write timeouts may not be long enough. See the [networking configuration doc](/configure/advanced-networking#setting-custom-readwrite-timeouts) for how to increase the read/write timeouts.
* If your application sends very large headers as part of the response body, consider increasing the response header size, as documented [here](/configure/advanced-networking#header-size).
* If you are seeing dropped connections while redeploying, follow the instructions for [zero-downtime deployments](/configure/zero-downtime-deployments).
...
This is caused by the NGINX instance rejecting requests that are too large. See the [networking configuration doc](/configure/advanced-networking#client-max-body-size) for how to resolve these errors.
