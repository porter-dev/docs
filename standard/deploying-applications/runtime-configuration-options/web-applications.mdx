...
<Info>For more information about webhooks and auto-deployment, see the [docs on programmatic Docker deployments](/standard/deploying-applications/deploying-from-docker-registry/deploying#programmatic-updates)</Info>
...
Selecting this option will create a public endpoint for your application. If this option is selected but a custom domain is not specified, the application will use a [Porter domain](/standard/deploying-applications/https-and-domains/porter-domains).

#### Configure Custom Domain[](#configure-custom-domain "Direct link to heading")

Selecting this option will expand a section that allows you to configure a set of custom domains for your application. See the [docs on custom domains](/standard/deploying-applications/https-and-domains/custom-domains) for more information.
...
Custom health checks are endpoints that indicate if an application is healthy and ready to receive traffic. These are documented in detail in the [Zero-Downtime Deployments](/standard/deploying-applications/zero-downtime-deployments) guide.
...
The termination grace period is the number of seconds after `SIGTERM` is sent before the application will be forcefully killed. This is documented in detail in the [Zero-Downtime Deployments](/standard/deploying-applications/zero-downtime-deployments) guide.
