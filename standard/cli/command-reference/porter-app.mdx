...
Runs a command inside a connected cluster container. By default, this spins up an ephemeral copy of your container that is deleted when the command completes.
...
| `--allow-concurrent` | | Allow concurrent job runs when using `--job` |
...
| `--revision` | The revision number to get logs for (default: -1) |
...
porter app build my-app --build-method pack --builder heroku/buildpacks:20 --attach-buildpacks heroku/nodejs
```
</CodeGroup>
