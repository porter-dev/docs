...
By default, the command will stream new logs. Setting the `--historical` flag will switch to pulling historical logs:

```
porter app logs [APP_NAME] --historical
```
...
```yaml
# --start -> the start time (in UTC) - defaults to 24 hours ago
porter app logs [APP_NAME] --historical --start 2021-01-01T00:00:00Z

# --end -> the end time (in UTC) - defaults to now
porter app logs [APP_NAME] --historical --end 2021-01-01T00:00:00Z

# --limit -> the number of lines of historical logs to return - defaults to 100
porter app logs [APP_NAME] --historical --limit=[LIMIT]

# --newest-first -> return the newest historical logs first - defaults to true
porter app logs [APP_NAME] --historical --newest-first=false
```
