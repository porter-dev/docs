...
1. Create a new role through the Azure CLI
...
For this reason, we need to create a new role that combines the Contributor scope along with permissions to create role assignments. To get started, copy the following JSON into a file called `porter-aks-role.json`, replacing all instances of `<SUBSCRIPTION_ID>` with your Azure subscription ID:
...
```
{
    ...
    "description": "Grants full access to manage all resources for a Porter AKS cluster.",
    "id": "/subscriptions/<SUBSCRIPTION_ID>/providers/Microsoft.Authorization/roleDefinitions/porter-aks-restricted",
    "isCustom": true,
...
After saving the file, create the role through the Azure CLI by running the following command in the same directory as the file you just saved:

```
$ az role definition create --role-definition @porter-aks-role.json
```
...
As before, make sure to replace `<SUBSCRIPTION_ID>` with your Azure subscription ID.
...
![API Permissions](https://github.com/porter-dev/porter/assets/66391417/3b633d51-f205-4136-9b81-7b6d54abaa5c)
...
![Usage Quotas](https://github.com/porter-dev/porter/assets/66391417/62e75cda-f8c2-48d8-9b62-83601c5c18ee)

While the exact virtual machines provisioned by Porter will depend on your selected region's availability, the following table covers all the virtual machine types that Porter may provision along with recommended initial quota limits:

|         Resource Family       | Recommended Quota  |
| ----------------------------- | ------ |
| Total Regional vCPUs          |   20   |
| Standard Basv2 Family vCPUs   |   20   |
...
![Azure credentials](https://github.com/porter-dev/porter/assets/66391417/d52e86f3-4157-4cd8-a9d4-2c824b6c2ce3)
...
![Cluster settings](https://github.com/porter-dev/porter/assets/66391417/87e7f207-c7cc-4f4f-a7cb-de4366e1c839 "Direct link to heading")
...
![Azure portal resource groups](https://github.com/porter-dev/porter/assets/66391417/ddfe6f23-935f-432c-a23c-799012d96067 "Screen Shot 2021-01-09 at 3.02.07 PM.png")