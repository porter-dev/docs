...
<AccordionGroup>
  ...
  <Accordion title="Cannot delete because credentials are missing or expired">
    ...
    **GCP**: Workload Identity Federation tokens don't expire — they're minted on demand. If Porter can't authenticate, either the `porter-pool-*` Workload Identity Pool or the `porter-manager-*` service account was deleted. Re-run the connect flow in **Integrations** → **GCP** to recreate them, then delete the cluster.
