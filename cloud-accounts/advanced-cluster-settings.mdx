...
| Setting | Description |
|---------|-------------|
| **DNS provider** | The DNS provider used to solve ACME DNS-01 challenges for private load balancer ingress. Choose **Cloudflare** or **AWS Route53**. |

<Tabs>
  <Tab title="Cloudflare">

When **Cloudflare** is selected, provide an API token that has permission to create and delete `TXT` records on the zones used by your private ingress hostnames.

| Setting | Description |
|---------|-------------|
| **DNS credentials** | Cloudflare API token. Must have permission to create and delete `TXT` records on the zones used by your private ingress hostnames. |
...
When **AWS Route53** is selected, no credentials are required — Porter authenticates to Route53 through the cluster's pod identity. You only need to specify the domain whose hosted zone Porter should write challenge records to.

| Setting | Description |
|---------|-------------|
| **Route53 domain** | The domain this private load balancer issues certificates for. Must be managed in a Route53 hosted zone in the same AWS account as the cluster. |

Porter resolves the domain to its hosted zone at install time and uses pod identity to create and delete the `TXT` records needed for ACME DNS-01 challenges.
