...
The domain you provide must already exist as a **public** hosted zone in the cluster's AWS account, and the zone name must match the domain exactly. If only a parent zone exists — for example, you set the domain to `internal.example.com` but only `example.com` is hosted in Route53 — Porter refuses to scope cert-manager to the parent. Delegate `internal.example.com` into its own hosted zone first.

Private hosted zones and duplicate public hosted zones with the same name are not supported. Remove duplicates before saving.
