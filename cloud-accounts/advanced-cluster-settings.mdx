...
<Info>
  If you are setting up an ALB now, use [Application load balancer](#application-load-balancer) below instead. This setting is kept for clusters already running on it. The two configure different load balancers, so nothing here carries over, and IP allowlisting on the additional ALB is done with WAF rules rather than a CIDR list.
</Info>

### Application load balancer

This is how to configure an ALB going forward. The setting adds an additional public ALB next to the cluster default, rather than replacing it. Each service picks which load balancer serves it, so you can move domains over one at a time.

| Setting | Description |
|---------|-------------|
| **Add application load balancer** | Provisions a public layer 7 ALB in front of the cluster. It listens on 80 and 443, terminates TLS at the edge, and does not redirect HTTP to HTTPS. |
| **Root domains** | The domains this load balancer serves, one per row (for example, `alb.example.com`). Porter requests an ACM certificate per root covering `*.<root>`, so any subdomain of it can be served. At least one is required. |
| **DNS provider** | The provider hosting the root domains, used to validate the certificate. **Cloudflare** and **AWS Route53** are supported. |
...
ACM validates through DNS, and the ALB cannot serve until its certificate is `ISSUED`. While it is pending, the cluster update keeps retrying and the error lists the records ACM is waiting on.

With **Route53**, Porter writes those records for you. Each root domain needs a hosted zone in the cluster's AWS account.

With **Cloudflare**, you write them yourself. Take the `CNAME name` and `CNAME value` from the certificate in the ACM console, in the cluster's region, and add them to your zone as described in [DNS validation](/security-and-compliance/configuring-alb#dns-validation).

#### Pointing DNS at the ALB

The ALB has its own address, so records pointing at the default load balancer are untouched. For each root domain, point a wildcard record (for example, `*.alb.example.com`) at the ALB. On AWS that address is a hostname, so use a `CNAME` or a Route53 alias. Once the ALB is provisioned, the address shows up under **Custom domains** in a service's **Networking** tab.

#### Web application firewall

Porter creates the web ACL with a default action of allow and leaves the rules to you. Add managed rule groups, rate-based rules, or IP sets in the AWS WAF console and Porter will not overwrite them. Only Regional WAFv2 is supported.

Turning the setting off detaches the web ACL without deleting it, so your rules survive if you re-enable it later.

#### Routing a service through the ALB

Once the ALB is serving, each web service's **Networking** tab offers a **Public ALB** option next to **Public LB**. In `porter.yaml`, use [`loadBalancers`](/applications/configuration-as-code/services/web-service#loadbalancers):
...
Domains on an ALB-served service must fall under a registered root. A service can serve both **Public LB** and **Public ALB** at once, which is how you cut over without downtime: keep both, move the domain's DNS record to the ALB, then unselect **Public LB**. It stops serving on the next deploy.
