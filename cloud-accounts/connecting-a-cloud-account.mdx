...
<Tabs>
  ...
  <Tab title="GCP">
    Porter connects to GCP using [Workload Identity Federation](https://cloud.google.com/iam/docs/workload-identity-federation) — a secure connection that does not require any static keys. Setup runs in [Google Cloud Shell](https://cloud.google.com/shell) with a single command Porter generates for your project.
    ...
    <Steps>
      ...
      <Step title="Run the setup in Cloud Shell">
        ...
        - A `porter-pool-*` Workload Identity Pool with a trust policy scoped to your project
