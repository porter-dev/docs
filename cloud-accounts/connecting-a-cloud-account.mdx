...
<Tabs>
  ...
  <Tab title="GCP">
    Porter connects to GCP using [Workload Identity Federation](https://cloud.google.com/iam/docs/workload-identity-federation) — no long-lived service account keys, no JSON files to manage. Porter's AWS-side identity exchanges short-lived (≤1 hour) tokens with your GCP project on demand.

    ## Prerequisites

    - An active [billing account](https://console.cloud.google.com/billing) attached to your GCP project. Porter cannot provision infrastructure without one.
    - You are the project owner, or you have these roles on the project:
      - `roles/serviceusage.serviceUsageAdmin`
      - `roles/iam.serviceAccountAdmin`
      - `roles/resourcemanager.projectIamAdmin`
      - `roles/iam.workloadIdentityPoolAdmin`
      - `roles/storage.admin` (first run only — for the Terraform state bucket)

    ## Connect Your GCP Project

    <Steps>
      <Step title="Enter your GCP Project ID">
        In Porter, select GCP and click **Connect**. You can find your project ID at the top of any page in the [GCP Console](https://console.cloud.google.com).
      </Step>
      <Step title="Copy the setup command">
        Porter generates a one-line setup command tied to this specific integration. Copy it from the dialog.
      </Step>
      <Step title="Run the command in Cloud Shell">
        Click **Open Cloud Shell** to launch [Google Cloud Shell](https://cloud.google.com/shell) with the [porter-dev/gcp-onboarding](https://github.com/porter-dev/gcp-onboarding) repository pre-loaded. Paste the command into the shell and press Enter.

        The script runs Terraform in your project and creates:
        - A `porter-manager-*` service account that Porter impersonates
        - A `porter-pool-*` Workload Identity Pool with a trust policy scoped to Porter's AWS identity and your tenant ID
        - Five Google APIs enabled: Cloud Resource Manager, IAM, IAM Credentials, STS, and Service Usage

        Setup takes about 30 seconds.
      </Step>
      <Step title="Wait for verification">
        Porter's dashboard polls the connection automatically. Once the bootstrap completes, Porter finishes enabling the remaining APIs, grants the heavier IAM roles, and marks the cloud account connected. You can close the dialog once the status banner turns green.
      </Step>
    </Steps>

    <Info>
    The setup is idempotent. If anything fails partway through, just re-run the command — Terraform state lives in a GCS bucket inside your project (`porter-tfstate-<project-id>`) and re-runs pick up from where they left off.
    </Info>
    ...
    The bootstrap initially grants three roles to the `porter-manager` service account — just enough for Porter to enable the remaining APIs and grant itself the heavier per-service roles:

    - `roles/serviceusage.serviceUsageAdmin`
    - `roles/resourcemanager.projectIamAdmin`
    - `roles/iam.serviceAccountAdmin`

    Once federation is verified, Porter automatically grants the rest (Compute Admin, Kubernetes Engine Admin, Artifact Registry Admin, Secret Manager Admin, etc.). The full Terraform module is open source: [porter-dev/gcp-onboarding](https://github.com/porter-dev/gcp-onboarding).

    ## Why Workload Identity Federation

    - **No long-lived secrets.** Federation issues short-lived (≤1 hour) tokens on demand. There is no key to rotate, store, or accidentally leak.
    - **Tight trust boundary.** The Workload Identity provider's attribute condition validates that each request comes from Porter's specific AWS account, the specific cluster control plane IAM role, and a session tagged with your tenant's external ID. A leak of the provider name and service account email cannot be turned into access without compromising Porter's AWS infrastructure.
    - **Auditable.** Every Terraform resource is open source. The `description` field on each resource is stamped with the Porter project ID and cloud account UUID for asset-inventory traceability.
    ...
    3. In Porter, navigate to **Integrations** → **Cloud accounts** and delete the GCP cloud account.
