...
<Tabs>
  ...
  <Tab title="GCP">
    Porter connects to GCP using [Workload Identity Federation](https://cloud.google.com/iam/docs/workload-identity-federation) (WIF). Porter's AWS identity exchanges short-lived (≤1 hour) tokens with your GCP project on demand — no service account keys to download, store, or rotate. Setup runs in [Google Cloud Shell](https://cloud.google.com/shell) with a single command Porter generates for your project.
    ...
    <Steps>
      ...
      <Step title="Run the setup in Cloud Shell">
        Click **Open Cloud Shell** to launch a new tab with the [porter-dev/gcp-onboarding](https://github.com/porter-dev/gcp-onboarding) repository pre-loaded. Paste the command into the shell and press Enter.
      ...
      <Step title="Wait for verification">
        Porter's dashboard polls the connection automatically. Once the bootstrap completes, Porter finishes enabling the remaining APIs, grants the heavier IAM roles to the service account, and marks the cloud account connected. The status banner turns green when you're done.
      </Step>
    </Steps>

    <Info>
    The setup is idempotent. If anything fails partway through, just re-run the command — Terraform state lives in a GCS bucket inside your project (`porter-tfstate-<project-id>`) and re-runs pick up where they left off.
    </Info>
    ...
    Your existing clusters keep authenticating with the JSON key while the new federation is being verified. Once verified, Porter swaps the credential atomically — there is no service interruption. You can then safely delete the JSON key from GCP.
