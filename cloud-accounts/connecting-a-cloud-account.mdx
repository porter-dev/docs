  ...
  <Tab title="Azure">
    ...
    <AccordionGroup>
      <Accordion title="Option 1: Automated setup (recommended)">
        Make sure the Azure CLI is installed and you've authenticated:

        ```bash
        az login
        ```

        Then download and run our setup script. The block below is a single copyable script — it downloads the setup script, makes it executable, and runs it. Optionally append your subscription ID as an argument.
